...
local function buildScavengeSkillContext(worker, siteProfile, skillEffects)
    ...
    for skillID, mix in pairs(secondaryWeights) do
        local safeMix = Output.clampNumber((tonumber(mix) or 0) * (tonumber(defaults.secondarySkillWeightScale) or 0.45), 0, 1)
        if safeMix > 0 then
...
local function getRuleSkillRating(rule, skillContext)
    ...
    for skillID, affinity in pairs(weights) do
        ...
        if skillID == skillContext.primarySkillID then
            ...
            mix = Output.clampNumber(skillContext.siteMix[skillID] or 0, 0, 1)
        end
...
local function getRuleSkillWeightMultiplier(rule, skillContext)
    ...
    return Output.clampNumber(multiplier, 0, 3), rating
end
...
local function buildWeightedScavengeEntries(loadout, siteProfile, skillEffects, skillContext, onlyRare)
    ...
    for _, rule in ipairs(Config.ScavengeLootRules or {}) do
        ...
        if minTier <= math.max(0, tonumber(loadout and loadout.tier) or 0) then
            ...
            if isEligible then
                local pool = Output.getCandidates(rule.tags)
                if #pool > 0 then
                    ...
                    local weight = Output.applyWeightMultiplier(rule.weight, combinedMultiplier)
                    if weight > 0 then
                        totalWeight = totalWeight + weight
                        entries[#entries + 1] = {
                            rule = rule,
    ...
    failureWeight = Output.applyWeightMultiplier(failureWeight, skillEffects and skillEffects.botchChanceMultiplier or 1)
...
local function resolveScavengeQuality(rule, selected, skillEffects, skillContext)
    ...
    if rule and rule.botchOutcome then
        ...
        botchChance = Output.clampNumber(botchChance, 0, 0.55)
        if Output.rollChance(botchChance) then
    ...
    if skillRating >= excellentThreshold then
        ...
        if Output.rollChance(Output.clampNumber(excellentChance, 0, 0.30)) then
    ...
    if skillRating >= difficulty then
        ...
        if Output.rollChance(Output.clampNumber(cleanChance, 0, 0.50)) then
...
local function buildWasteScavengeEntry(loadout, skillEffects)
    local pool = Output.getCandidates({ "Quality.Waste" })
    if #pool <= 0 then
        return nil
    end

    local qty = Output.applyQuantityMultiplier(getRuleQuantity({
        minQty = 1,
        maxQty = 2,
        bulkBonus = 1,
        bundleBonus = 1
...
local function resolveScavengeSelection(selected, loadout, skillEffects, skillContext)
    ...
    local qty = Output.applyQuantityMultiplier(getRuleQuantity(selected.rule, loadout), skillEffects.yieldMultiplier)
    if quality == "clean" then
        qty = Output.applyQuantityMultiplier(qty, 1.15)
    elseif quality == "excellent" then
        qty = Output.applyQuantityMultiplier(qty, selected.rule.isRare == true and 1.20 or 1.35)
    end
