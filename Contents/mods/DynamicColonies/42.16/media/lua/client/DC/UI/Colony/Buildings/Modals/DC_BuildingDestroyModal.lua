...
DC_BuildingDestroyModal = ISCollapsableWindow:derive("DC_BuildingDestroyModal")
DC_BuildingDestroyModal.instance = DC_BuildingDestroyModal.instance or nil

function DC_BuildingDestroyModal:initialise()
    ISCollapsableWindow.initialise(self)
    self:setResizable(false)
end

function DC_BuildingDestroyModal:createChildren()
...
function DC_BuildingDestroyModal:onConfirmClicked()
...
function DC_BuildingDestroyModal:onCancelClicked()
    self:close()
end

function DC_BuildingDestroyModal:close()
    self:setVisible(false)
    self:removeFromUIManager()
    if DC_BuildingDestroyModal.instance == self then
        DC_BuildingDestroyModal.instance = nil
    end
end

function DC_BuildingDestroyModal:new(x, y, width, height)
...
function DC_BuildingDestroyModal.Open(args)
    args = args or {}
    if DC_BuildingDestroyModal.instance then
        DC_BuildingDestroyModal.instance:close()
    end
    ...
    local modal = DC_BuildingDestroyModal:new(x, y, width, height)
    ...
    modal:bringToTop()
    DC_BuildingDestroyModal.instance = modal
    return modal
end

return DC_BuildingDestroyModal
