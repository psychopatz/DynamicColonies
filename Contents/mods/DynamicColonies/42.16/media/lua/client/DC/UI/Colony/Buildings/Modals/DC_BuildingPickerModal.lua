...
local PickerCarousel = ISPanel:derive("DC_BuildingPickerModal_Carousel")
...
function PickerCarousel:render()
    ...
    for index, option in ipairs(self.options or {}) do
        ...
        if cardX + metrics.width >= 0 and cardX <= self.width then
            ...
            local border = selected and DC_BuildingsUIUtils.Colors.selectedBorder or DC_BuildingsUIUtils.Colors.defaultBorder
            ...
            local texture = DC_BuildingsUIUtils.GetTexture(option.iconPath)
            if texture then
            ...
            local subtitle = DC_BuildingsUIUtils.GetOptionStatusLabel and DC_BuildingsUIUtils.GetOptionStatusLabel(option)
...
DC_BuildingPickerModal = ISCollapsableWindow:derive("DC_BuildingPickerModal")
DC_BuildingPickerModal.instance = DC_BuildingPickerModal.instance or nil

function DC_BuildingPickerModal:initialise()
    ISCollapsableWindow.initialise(self)
    self:setResizable(false)
end

function DC_BuildingPickerModal:getSelectedOption()
    return self.options and self.options[self.selectedIndex or 1] or nil
end

function DC_BuildingPickerModal:updateDetailText()
    local option = self:getSelectedOption()
    if self.detailText then
        self.detailText:setText(DC_BuildingsUIUtils.BuildOptionDetailText(option))
...
function DC_BuildingPickerModal:scrollCards(delta)
    if self.carousel then
        self.carousel:scrollBy(delta)
    end
end

function DC_BuildingPickerModal:createChildren()
...
function DC_BuildingPickerModal:onScrollLeft()
    self:scrollCards(-180)
end

function DC_BuildingPickerModal:onScrollRight()
    self:scrollCards(180)
end

function DC_BuildingPickerModal:onConfirmClicked()
    local option = self:getSelectedOption()
...
function DC_BuildingPickerModal:onCancelClicked()
    self:close()
end

function DC_BuildingPickerModal:close()
    self:setVisible(false)
    self:removeFromUIManager()
    if DC_BuildingPickerModal.instance == self then
        DC_BuildingPickerModal.instance = nil
    end
end

function DC_BuildingPickerModal:new(x, y, width, height)
...
function DC_BuildingPickerModal.Open(args)
    args = args or {}
    if DC_BuildingPickerModal.instance then
        DC_BuildingPickerModal.instance:close()
    end
    ...
    local modal = DC_BuildingPickerModal:new(x, y, width, height)
    ...
    modal:bringToTop()
    DC_BuildingPickerModal.instance = modal
    return modal
end

return DC_BuildingPickerModal
